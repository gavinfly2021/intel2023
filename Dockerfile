...
RUN echo 'Ghost' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/Ghost.sh

RUN echo 'cd ' >>/Ghost.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/Ghost.sh

RUN echo 'cd /noVNC-1.2.0' >>/Ghost.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/Ghost.sh

RUN chmod 755 /Ghost.sh
